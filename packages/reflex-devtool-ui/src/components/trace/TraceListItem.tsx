...
function EventTraceItem({ item, showBadges, showParams }: Omit<TraceListItemProps, 'selected'>) {
...
function RenderTraceItem({ item, showBadges }: Omit<TraceListItemProps, 'selected'>) {
  return (
    <div className="flex items-center justify-between px-2 py-2 text-sm">
...
export default function TraceListItem({ item, selected, showBadges, showParams }: TraceListItemProps) {
  ...
  return (
    <li className={`hover:bg-base-300 border-b border-base-300 cursor-pointer ${selected ? 'bg-base-300' : ''}`} onClick={handleClick}>
      {isEvent ? (
        <EventTraceItem item={item} showBadges={showBadges} showParams={showParams} />
      ) : (
        <RenderTraceItem item={item} showBadges={showBadges} />
      )}
    </li>
  );
} 